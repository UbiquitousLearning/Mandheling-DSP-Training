...
Express::VARP _NITI_LOSS_CrossEntropy(Express::VARP predicts, Express::VARP ascale, Express::VARP oneHotTargets) {
    // MNN_ASSERT(predicts->getInfo()->dim == oneHotTargets->getInfo()->dim);
    auto loss = _NITI_LOSS_SUM(predicts, ascale, _Cast<int32_t>(oneHotTargets), 64, 10, 1, 1);
    return loss;
}

Express::VARP _NITI_CrossEntropy(Express::VARP predicts, Express::VARP oneHotTargets) {
    MNN_ASSERT(predicts->getInfo()->dim.size() == 2);
    MNN_ASSERT(predicts->getInfo()->dim == oneHotTargets->getInfo()->dim);
    auto loss = _Negative(_ReduceMean(_ReduceSum(  _Log(predicts) *  _Cast<int32_t>(oneHotTargets), {1}), {}));
    return loss;
}

Express::VARP _NITI_CrossEntropy_FAKE(Express::VARP predicts, Express::VARP oneHotTargets) {
    return predicts;
}
