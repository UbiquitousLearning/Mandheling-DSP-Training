...
VARP _NITI_Transpose_INT8(VARP x, INTS perm) {
    return _Cast<int8_t>(_Transpose(_Cast<int32_t>(x), perm));
}
...
std::vector<VARP> _NITI_Conv_Int8(std::vector<int8_t>&& weight, VARP ascale, int8_t wscale, VARP x, INTS channel, INTS kernelSize,
                              PaddingMode pad, INTS stride, INTS dilate, int group, INTS pads, int nbits) {
    std::unique_ptr<OpT> convOp(new OpT);
    convOp->type = OpType_NITI_CONV_Int8;
    // if (channel[0] == channel[1] && channel[0] == group) {
    //     convOp->type = OpType_DepthwiseConvInt8;
    // }
    convOp->main.type  = OpParameter_NITI_CONV_Int8;
    convOp->main.value = new NITI_CONV_Int8T;
    auto conv2D        = convOp->main.AsNITI_CONV_Int8();
    conv2D->common.reset(new Convolution2DCommonT);
    conv2D->common->padMode     = _convertPadMode(pad);
    conv2D->common->padX        = pads[0];
    conv2D->common->padY        = pads[1];
    ...
    conv2D->wscale = wscale;
    conv2D->weight = std::move(weight);
    conv2D->nbits = nbits;
    std::vector<VARP> res(2);
    auto expr = Expr::create(convOp.get(), {x,ascale}, 2);

    res[0] = Variable::create(expr, 0);
    res[1] = Variable::create(expr, 1);
    return res;
}
std::vector<VARP> _NITI_Conv_Int8(VARP weight, VARP ascale, VARP wscale, VARP x,
                              PaddingMode pad, INTS stride, INTS dilate, int group, INTS pads, int nbits) {
    std::unique_ptr<OpT> convOp(new OpT);
    convOp->type = OpType_NITI_CONV_Int8;
    auto shape   = weight->getInfo();
    ...
    std::vector<VARP> res(2);
    auto expr = Expr::create(convOp.get(), {x, weight, ascale, wscale}, 2);
...
std::vector<VARP> _NITI_Maxpool_Int8(VARP x, VARP ascale, INTS kernel, INTS stride, PaddingMode pad, INTS pads) {
    std::unique_ptr<OpT> pool(new OpT);
    pool->type       = OpType_NITI_Maxpool_Int8;
    pool->main.type  = OpParameter_NITI_Pool_Int8;
    pool->main.value = new NITI_Pool_Int8T;
    // if (kernel[0] == -1 && kernel[1] == -1) {
    //     pool->main.AsPool()->isGlobal = true;
    // }
    pool->main.AsNITI_Pool_Int8()->padX = 0;
    pool->main.AsNITI_Pool_Int8()->padY = 0;
    if (pads.size() >= 2) {
        pool->main.AsNITI_Pool_Int8()->padX = pads[0];
        pool->main.AsNITI_Pool_Int8()->padY = pads[1];
    }
    pool->main.AsNITI_Pool_Int8()->padType = _convertPoollingPadMode(pad);
    pool->main.AsNITI_Pool_Int8()->kernelX = kernel[0];
    pool->main.AsNITI_Pool_Int8()->kernelY = kernel[1];
    pool->main.AsNITI_Pool_Int8()->strideX = stride[0];
    pool->main.AsNITI_Pool_Int8()->strideY = stride[1];
    pool->main.AsNITI_Pool_Int8()->type    = PoolType_MAXPOOL;

    auto expr = Expr::create(pool.get(), {_Convert(x, NHWC), ascale}, 2);
    std::vector<VARP> res(2);
    res[0] = Variable::create(expr, 0);
    res[1] = Variable::create(expr, 1);
    return res;
...
VARP _NITI_Softmax(VARP logits, VARP ascale, int axis) {
    std::unique_ptr<OpT> softmax(new OpT);
    softmax->type                = OpType_NITI_SOFTMAX_Int8;
    softmax->main.type           = OpParameter_Axis;
    softmax->main.value          = new AxisT;
    softmax->main.AsAxis()->axis = axis;
    return (Variable::create(Expr::create(softmax.get(), {logits, ascale})));
}
VARP _NITI_LOSS_SUM(VARP logits, VARP ascale, VARP oneHotTargets, int batch, int channel, int height, int width) {
    std::unique_ptr<OpT> softmax(new OpT);
    softmax->type                = OpType_NITI_LOSS_Int8;
    softmax->main.type           = OpParameter_NITI_LOSS_Int8;
    softmax->main.value          = new NITI_LOSS_Int8T;

    softmax->main.AsNITI_LOSS_Int8()->batch = batch;
    softmax->main.AsNITI_LOSS_Int8()->channel = channel;
    softmax->main.AsNITI_LOSS_Int8()->height = height;
    softmax->main.AsNITI_LOSS_Int8()->width = width;

    return (Variable::create(Expr::create(softmax.get(), {logits, ascale, oneHotTargets},1)));
}

VARP _NITI_Relu_Int8(VARP x) {
    std::unique_ptr<OpT> softmax(new OpT);
    softmax->type                = OpType_NITI_Relu_Int8;

    return (Variable::create(Expr::create(softmax.get(), {x},1)));
}

VARP _NITI_ReluGrad_Int8(VARP input, VARP x) {
    std::unique_ptr<OpT> softmax(new OpT);
    softmax->type                = OpType_NITI_ReluGrad_Int8;

    return (Variable::create(Expr::create(softmax.get(), {input, x},1)));
}

VARP _NITI_Pad_Int8(VARP input, int padpos) {
    std::unique_ptr<OpT> pad(new OpT);
    pad->type                = OpType_NITI_PAD_Int8;
    pad->main.type           = OpParameter_NITI_PAD_Int8;
    pad->main.value          = new NITI_PAD_Int8T;

    pad->main.AsNITI_PAD_Int8()->pad = padpos;

    return (Variable::create(Expr::create(pad.get(), {input},1)));
}

VARP _NITI_ConvertChannel1NC4HW4ToNCHW_Int8(VARP x) {
    std::unique_ptr<OpT> c1(new OpT);
    c1->type                = OpType_NITI_ConvertChannel1NC4HW4ToNCHW_Int8;

    return (Variable::create(Expr::create(c1.get(), {x},1)));
}

std::vector<VARP> _NITI_ConvMinMax_Int8(VARP weight, VARP ascale, VARP wscale, VARP x,
                              PaddingMode pad, INTS stride, INTS dilate, int group, INTS pads, int nbits) {
    std::unique_ptr<OpT> convOp(new OpT);
    convOp->type = OpType_NITI_CONVMaxMin_Int8;
    auto shape   = weight->getInfo();
    if (NHWC == shape->order) {
        weight = _Transpose(weight, {0, 3, 1, 2});
        shape  = weight->getInfo();
    ...
    std::vector<VARP> res(3);
    auto expr = Expr::create(convOp.get(), {x, weight}, 3);

    res[0] = Variable::create(expr, 0);
    res[1] = Variable::create(expr, 1);
    res[2] = Variable::create(expr, 2);
    return res;
}

VARP _NITI_LeftPoolGrad(VARP x, INTS shape, INTS stride ) {
    std::unique_ptr<OpT> pool(new OpT);
    pool->type       = OpType_NITI_LeftPoolGrad_Int8;
    pool->main.type  = OpParameter_NITI_Pool_Int8;
    pool->main.value = new NITI_Pool_Int8T;
    ...
    pool->main.AsNITI_Pool_Int8()->oh = shape[0];
    pool->main.AsNITI_Pool_Int8()->ow = shape[1];

    return Variable::create(Expr::create(pool.get(), {x}, 1));
}

// DSP算子实现
std::vector<VARP> _NITI_DSP_Conv_Int8(VARP weight, VARP ascale, VARP wscale, VARP x,
                              PaddingMode pad, INTS stride, INTS dilate, int group, INTS pads, int nbits) {
    std::unique_ptr<OpT> convOp(new OpT);
    convOp->type = OpType_NITI_DSP_CONV_Int8;
    auto shape   = weight->getInfo();

    auto channel    = std::vector<int>{shape->dim[3], shape->dim[2]};
    auto kernelSize = std::vector<int>{shape->dim[1], shape->dim[0]};
    convOp->main.type  = OpParameter_NITI_CONV_Int8;
    convOp->main.value = new NITI_CONV_Int8T;
    auto conv2D        = convOp->main.AsNITI_CONV_Int8();
    conv2D->common.reset(new Convolution2DCommonT);
...
std::vector<VARP> _NITI_DSP_Conv_Int8_SIMU(VARP weight, VARP ascale, VARP wscale, VARP x, VARP min, VARP max,
                              PaddingMode pad, INTS stride, INTS dilate, int group, INTS pads, int nbits) {
    std::unique_ptr<OpT> convOp(new OpT);
    convOp->type = OpType_NITI_DSP_CONV_Int8;
    auto shape   = weight->getInfo();
    ...
    std::vector<VARP> res(2);
    auto expr = Expr::create(convOp.get(), {x, weight, ascale, wscale, min, max}, 2);
...
VARP _NITI_DSP_Relu_Int8(VARP x) {
    std::unique_ptr<OpT> softmax(new OpT);
    softmax->type                = OpType_NITI_DSP_RELU_Int8;

    return (Variable::create(Expr::create(softmax.get(), {x},1)));
}
std::vector<VARP> _NITI_DSP_Maxpool_Int8(VARP x, VARP ascale, INTS kernel, INTS stride, PaddingMode pad, INTS pads) {
    std::unique_ptr<OpT> pool(new OpT);
    pool->type       = OpType_NITI_DSP_MAXPOOL_Int8;
    pool->main.type  = OpParameter_NITI_Pool_Int8;
    pool->main.value = new NITI_Pool_Int8T;
    // if (kernel[0] == -1 && kernel[1] == -1) {
    //     pool->main.AsPool()->isGlobal = true;
    ...
    auto expr = Expr::create(pool.get(), {x, ascale}, 2);
    std::vector<VARP> res(2);
    res[0] = Variable::create(expr, 0);
    res[1] = Variable::create(expr, 1);
    return res;
...
VARP _NITI_DSP_Reshape(VARP x, INTS shape, Dimensionformat original_format) {
    std::unique_ptr<OpT> reshape(new OpT);
    reshape->type                      = OpType_NITI_DSP_RESHAPE_Int8;
    reshape->main.type                 = OpParameter_Reshape;
    reshape->main.value                = new ReshapeT;
    reshape->main.AsReshape()->dims    = shape;
    reshape->main.AsReshape()->dimType = (MNN_DATA_FORMAT)Utils::convertFormat(original_format);
...
VARP _NITI_DSP_ReshapeGrad(VARP x, INTS shape, Dimensionformat original_format) {
    std::unique_ptr<OpT> reshape(new OpT);
    reshape->type                      = OpType_NITI_DSP_RESHAPEGrad_Int8;
    reshape->main.type                 = OpParameter_Reshape;
    reshape->main.value                = new ReshapeT;
    reshape->main.AsReshape()->dims    = shape;
    reshape->main.AsReshape()->dimType = (MNN_DATA_FORMAT)Utils::convertFormat(original_format);
...
VARP _NITI_DSP_ReluGrad_Int8(VARP input, VARP x) {
    std::unique_ptr<OpT> softmax(new OpT);
    softmax->type                = OpType_NITI_DSP_RELUGRAD_Int8;

    return (Variable::create(Expr::create(softmax.get(), {input, x},1)));
}

VARP _NITI_DSP_Transpose(VARP x, INTS perm) {
    auto permVar = _Const((const void*)perm.data(), {static_cast<int>(perm.size())}, NHWC, halide_type_of<int>());
    return _NITI_DSP_Transpose(x, permVar);
}

VARP _NITI_DSP_Transpose(VARP x, VARP perm) {
    std::unique_ptr<OpT> transpose(new OpT);
    transpose->type                      = OpType_NITI_DSP_TRANSPOSE_Int8;
    transpose->main.type                 = OpParameter_Transpose;
    transpose->main.value                = new TransposeT;
    transpose->main.AsTranspose()->Tperm = DataType_DT_INT32;
    return (Variable::create(Expr::create(std::move(transpose), {x, perm})));
...
VARP _NITI_DSP_WeightRotate180_Int8(VARP input) {
    std::unique_ptr<OpT> softmax(new OpT);
    softmax->type                = OpType_NITI_DSP_WEIGHTROTATE180_REF_Int8;

    return (Variable::create(Expr::create(softmax.get(), {input},1)));
}

VARP _NITI_DSP_Pad_Int8(VARP input, int padpos) {
    std::unique_ptr<OpT> pad(new OpT);
    pad->type                = OpType_NITI_DSP_PAD_Int8;
    pad->main.type           = OpParameter_NITI_PAD_Int8;
    pad->main.value          = new NITI_PAD_Int8T;
...
VARP _NITI_DSP_LeftPoolGrad_Deconv(VARP x, INTS shape, INTS stride ) {
    std::unique_ptr<OpT> pool(new OpT);
    pool->type       = OpType_NITI_DSP_LEFTPOOLGRAD_DECONV_Int8;
    pool->main.type  = OpParameter_NITI_Pool_Int8;
    pool->main.value = new NITI_Pool_Int8T;
...
VARP _NITI_DSP_LeftPoolGrad_Gradient(VARP x, INTS shape, INTS stride ) {
    std::unique_ptr<OpT> pool(new OpT);
    pool->type       = OpType_NITI_DSP_LEFTPOOLGRAD_GRADIENT_Int8;
    pool->main.type  = OpParameter_NITI_Pool_Int8;
    pool->main.value = new NITI_Pool_Int8T;
...
VARP _NITI_DSP_Binary_Add(VARP x, VARP y) {
    std::unique_ptr<OpT> binary(new OpT);
    binary->type       = OpType_NITI_DSP_BINARY_Int8;
    binary->main.type  = OpParameter_BinaryOp;
    binary->main.value = new BinaryOpT;
    
    binary->main.AsBinaryOp()->opType = BinaryOpOperation_ADD;

    return Variable::create(Expr::create(binary.get(), {x,y}, 1));
}

VARP _NITI_DSP_NOP(VARP x) {
    std::unique_ptr<OpT> nop(new OpT);
    nop->type       = OpType_NITI_DSP_NOP_Int8;

    return Variable::create(Expr::create(nop.get(), {x}, 1));
}
