//
//  NITI_DSPParallelGradientConv_Int8.cpp
//  MNN
//
//  Created by xudaliang on 2022/1/2.
//  
//
...
NITI_DSPParallelGradientConv_Int8::~NITI_DSPParallelGradientConv_Int8() {
    // Do nothing
    // hexagon_nn_->hexagon_nn_global_teardown();
}
NITI_DSPParallelGradientConv_Int8::NITI_DSPParallelGradientConv_Int8(Backend* backend, const NITI_CONV_Int8* convOp, const MNN::Op* op) : CPUConvolution(convOp->common(), backend) {
...
ErrorCode NITI_DSPParallelGradientConv_Int8::onResize(const std::vector<Tensor*>& inputs, const std::vector<Tensor*>& outputs) {
    ...
    if(getDSPExecuteMode() && !static_cast<CPUBackend*>(backend())->build_graph && static_cast<CPUBackend*>(backend())->parallel_mode) {
...
ErrorCode NITI_DSPParallelGradientConv_Int8::onExecute(const std::vector<Tensor*>& inputs, const std::vector<Tensor*>& outputs) {
    ...
    hexagon_nn_->hexagon_nn_append_node(graph_id_,     
            op_id,   OP_INPUT,   NN_PAD_NA,   empty_input.data(), 0,   _outputs0.data(), _outputs0.size());
    ...
    addInputTransposeLayer(hexagon_nn_,graph_id_,op_id,inputs[0], input_transposelayer_input, input_transposelayer_output, inputTransposeShape, &input_min, &input_max);
    ...
    } else if (mCommon->padX() == 0) {
         hexagon_nn_->hexagon_nn_append_node(graph_id_,     
            op_id,   OP_Supernode_8x8p32to8,   NN_PAD_VALID,   _inputs.data(), _inputs.size(),   _outputs.data(), _outputs.size());
    } else {
        MNN_ERROR("unspport pad mode\n");
    }
...
ErrorCode NITI_DSPParallelGradientConv_Int8::GlobalExecute(const std::vector<Tensor*>& inputs, const std::vector<Tensor*>& outputs) {
    ...
    if(conv_num == 1) {
        ...
        initDspGraph(static_cast<CPUBackend*>(backend())->global_hexagon_nn_, static_cast<CPUBackend*>(backend())->global_gradient_graph_id_);

        static_cast<CPUBackend*>(backend())->global_hexagon_nn_->hexagon_nn_append_node(static_cast<CPUBackend*>(backend())->global_gradient_graph_id_,     
    ...
    graph_id_ = static_cast<CPUBackend*>(backend())->global_gradient_graph_id_;
    ...
    inputTransposeShape.push_back(2);
    inputTransposeShape.push_back(0);
    addInputTransposeLayerWithInputOpIdWithIdx(hexagon_nn_,graph_id_, input_op_id, op_id, inputs[0], static_cast<CPUBackend*>(backend())->current_input_num, input_transposelayer_input, input_transposelayer_output, inputTransposeShape, &input_min, &input_max);
...
int NITI_DSPParallelGradientConv_Int8::conv_num = 0;

class NITI_DSPParallelGradientConv_Int8_Creator : public CPUBackend::Creator {
public:
    virtual Execution* onCreate(const std::vector<Tensor*>& inputs, const std::vector<Tensor*>& outputs,
                                const MNN::Op* op, Backend* backend) const override {

        auto convOp = op->main_as_NITI_CONV_Int8();
        return new NITI_DSPParallelGradientConv_Int8(backend, convOp, op);
    }
};

REGISTER_CPU_OP_CREATOR(NITI_DSPParallelGradientConv_Int8_Creator, OpType_NITI_DSP_PARALLEL_GRADIENTCONV_Int8);
} // namespace MNN
