...
NITI_DSPGradientConv_Int8::~NITI_DSPGradientConv_Int8() {
    // Do nothing
    // hexagon_nn_->hexagon_nn_global_teardown();
}
NITI_DSPGradientConv_Int8::NITI_DSPGradientConv_Int8(Backend* backend, const NITI_CONV_Int8* convOp, const MNN::Op* op) : CPUConvolution(convOp->common(), backend) {
...
ErrorCode NITI_DSPGradientConv_Int8::onResize(const std::vector<Tensor*>& inputs, const std::vector<Tensor*>& outputs) {
...
ErrorCode NITI_DSPGradientConv_Int8::onExecute(const std::vector<Tensor*>& inputs, const std::vector<Tensor*>& outputs) {
    ...
    if(mCommon->padX() > 0) {
        ...
        max_sizes1[1] = inputs[0]->buffer().dim[inputTransposeShape[1]].extent+2*mCommon->padX();
        max_sizes1[2] = inputs[0]->buffer().dim[inputTransposeShape[2]].extent+2*mCommon->padX();
...
ErrorCode NITI_DSPGradientConv_Int8::GlobalExecute(const std::vector<Tensor*>& inputs, const std::vector<Tensor*>& outputs) {
...
int NITI_DSPGradientConv_Int8::conv_num = 0;

class NITI_DSPGradientConv_Int8_Creator : public CPUBackend::Creator {
public:
    virtual Execution* onCreate(const std::vector<Tensor*>& inputs, const std::vector<Tensor*>& outputs,
                                const MNN::Op* op, Backend* backend) const override {

        auto convOp = op->main_as_NITI_CONV_Int8();
        return new NITI_DSPGradientConv_Int8(backend, convOp, op);
    }
};

REGISTER_CPU_OP_CREATOR(NITI_DSPGradientConv_Int8_Creator, OpType_NITI_DSP_GRADIENTCONV_Int8);
} // namespace MNN
