...
#include <math.h>
#include "backend/cpu/NITI_DSPRelu_Int8.hpp"
...
ErrorCode NITI_DSPRelu_Int8::onResize(const std::vector<Tensor *> &inputs, const std::vector<Tensor *> &outputs) {
...
ErrorCode NITI_DSPRelu_Int8::onExecute(const std::vector<Tensor *> &inputs, const std::vector<Tensor *> &outputs) {
    ...
    hexagon_nn_->hexagon_nn_append_node(graph_id_,     
            op_id,   OP_QuantizedRelu_8,   NN_PAD_NA,   relulayer_input.data(), relulayer_input.size(),   relulayer_output.data(), relulayer_output.size());
...
ErrorCode NITI_DSPRelu_Int8::GlobalExecute(const std::vector<Tensor *> &inputs, const std::vector<Tensor *> &outputs) {
    ...
    int input_op_id = backend()->get_Op_id(inputs[0], OpType_NITI_DSP_RELU_Int8);
...
NITI_DSPRelu_Int8::NITI_DSPRelu_Int8(Backend *b) : MNN::Execution(b) {
    // nothing to do
    if(!getDSPExecuteMode())
...
class NITI_DSPRelu_Int8_Creator : public CPUBackend::Creator {
public:
    virtual Execution *onCreate(const std::vector<Tensor *> &inputs, const std::vector<Tensor *> &outputs,
                                const MNN::Op *op, Backend *backend) const override {
        return new NITI_DSPRelu_Int8(backend);
    }
};

REGISTER_CPU_OP_CREATOR(NITI_DSPRelu_Int8_Creator, OpType_NITI_DSP_RELU_Int8);
