...
#include <math.h>
#include "backend/cpu/NITI_DSPReshape_Int8.hpp"
...
ErrorCode NITI_DSPReshape_Int8::onResize(const std::vector<Tensor *> &inputs, const std::vector<Tensor *> &outputs) {

    return NO_ERROR;
}

ErrorCode NITI_DSPReshape_Int8::onExecute(const std::vector<Tensor *> &inputs, const std::vector<Tensor *> &outputs) {
...
ErrorCode NITI_DSPReshape_Int8::GlobalExecute(const std::vector<Tensor *> &inputs, const std::vector<Tensor *> &outputs) {
    ...
    int input_op_id = backend()->get_Op_id(inputs[0], OpType_NITI_DSP_RESHAPE_Int8);
...
NITI_DSPReshape_Int8::NITI_DSPReshape_Int8(Backend *b, const MNN::Op* op) : MNN::Execution(b) {
    // nothing to do
    if(!getDSPExecuteMode())
...
class NITI_DSPReshape_Int8_Creator : public CPUBackend::Creator {
public:
    virtual Execution *onCreate(const std::vector<Tensor *> &inputs, const std::vector<Tensor *> &outputs,
                                const MNN::Op *op, Backend *backend) const override {
        return new NITI_DSPReshape_Int8(backend, op);
    }
};

REGISTER_CPU_OP_CREATOR(NITI_DSPReshape_Int8_Creator, OpType_NITI_DSP_RESHAPE_Int8);
