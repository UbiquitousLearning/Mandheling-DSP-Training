//
//  NITI_DSPGradientSplitBatchConv_Int8.cpp
//  MNN
//
//  Created by xudaliang on 2022/1/20.
//  
//

#include "backend/cpu/NITI_DSPGradientSplitBatchConv_Int8.hpp"
...
using Vecuint8 = MNN::Math::Vec<uint8_t, 4>;

namespace MNN {

NITI_DSPGradientSplitBatchConv_Int8::~NITI_DSPGradientSplitBatchConv_Int8() {
    // Do nothing
    // hexagon_nn_->hexagon_nn_global_teardown();
}
NITI_DSPGradientSplitBatchConv_Int8::NITI_DSPGradientSplitBatchConv_Int8(Backend* backend, const NITI_CONV_Int8* convOp, const MNN::Op* op) : CPUConvolution(convOp->common(), backend) {
...
ErrorCode NITI_DSPGradientSplitBatchConv_Int8::onResize(const std::vector<Tensor*>& inputs, const std::vector<Tensor*>& outputs) {
    ...
    inputSplitTensor.reset(Tensor::createDevice<uint8_t>({input->batch(), input->height(), input->width(), input->channel()}));
    success = backend()->onAcquireBuffer(inputSplitTensor.get(), Backend::DYNAMIC);
    if (!success) {
        return OUT_OF_MEMORY;
    }
    ...
    backend()->onReleaseBuffer(inputSplitTensor.get(), Backend::DYNAMIC);

    int split_num =  ichannel / minibatch;
...
ErrorCode NITI_DSPGradientSplitBatchConv_Int8::onExecute(const std::vector<Tensor*>& inputs, const std::vector<Tensor*>& outputs) {
    ...
    {
        uint8_t* tinputPtr = inputs[0]->host<uint8_t>();
        uint8_t* splitPtr = inputSplitTensor->host<uint8_t>();
        int split = ichannel / minibatch;
        int offset = inputSplitTensor->elementSize() / split;
        for(int ib = 0; ib<ibatch; ib++) {
            for(int ih = 0; ih < iheight; ih++) {
                for(int iw = 0; iw < iwidth; iw++) {

                    for(int ic = 0; ic < ichannel/minibatch; ic ++ ) {
                        
                        Vecuint8 originValue = Vecuint8::load(tinputPtr);

                        Vecuint8::save(splitPtr + offset*ic,  originValue);
                        tinputPtr += minibatch;
                    }
    ...
    actual_input.push_back(inputs[0]->batch());
    actual_input.push_back(inputs[0]->height());
    actual_input.push_back(inputs[0]->width());
    actual_input.push_back(inputs[0]->channel()/split_num);


    std::vector<int> actual_weight;
    actual_weight.push_back(inputs[1]->batch()/split_num);
    actual_weight.push_back(inputs[1]->height());
    actual_weight.push_back(inputs[1]->width());
    actual_weight.push_back(inputs[1]->channel());
    ...
    for (int n=0; n<split_num; n++) {
        ...
        if(mCommon->padX() > 0) {
            ...
            padlayer_input.back().output_idx = n*2;
            ...
            max_sizes1[3] = inputs[0]->buffer().dim[3].extent / split_num;
        ...
        addInputTransposeLayerWithInputOpIdWithIdxWithShape(hexagon_nn_,graph_id_, input_op_id, op_id,inputs[1], n*2+1, outdiff_transposelayer_input, outdiff_transposelayer_output, outdiffTransposeShape, &input_min, &input_max, actual_weight);

        addInputCastLayerWithIdxWithTransposeWithShape(hexagon_nn_, graph_id_, op_id, op_id, inputs[1], 0, castlayer_input, castlayer_output, OP_Quantized_CastInt8ToUInt8, outdiffTransposeShape, actual_weight);
        ...
        convOutputOpid.push_back(op_id);

        inputTransposeShape.resize(0);
        input_transposelayer_input.resize(0);
        input_transposelayer_output.resize(0);

        padtensor.resize(0);
        padlayer_input.resize(0);
        padlayer_output.resize(0);


        outdiffTransposeShape.resize(0);
        outdiff_transposelayer_input.resize(0);
        outdiff_transposelayer_output.resize(0);

        castlayer_input.resize(0);
        castlayer_output.resize(0);
    ...
    int accu_conv_opid = convOutputOpid[0];

    for(int n=1;n<split_num; n++) {

        addlayer_input.push_back(hexagon_nn_input());
        addlayer_input.back().src_id = accu_conv_opid;
        addlayer_input.back().output_idx = 0;

        addlayer_input.push_back(hexagon_nn_input());
        addlayer_input.back().src_id = convOutputOpid[n];
        addlayer_input.back().output_idx = 0;

        addConstInputTensor(hexagon_nn_, op_id, 0, addlayer_input, (uint8_t*)&input_min, graph_id_, 1, sizeof(float));
        addConstInputTensor(hexagon_nn_, op_id, 0, addlayer_input, (uint8_t*)&input_max, graph_id_, 1, sizeof(float));
        ...
        addlayer_output.push_back(hexagon_nn_output());
        addlayer_output.back().rank = 4;
        auto& max_sizes = addlayer_output.back().max_sizes;
        ...
        addlayer_output.back().elementsize = sizeof(uint8_t);
        ...
        max_size.push_back(1);
        max_size.push_back(1);
        addOutputTensor(max_size,addlayer_output, sizeof(float));
        addOutputTensor(max_size,addlayer_output, sizeof(float));


        hexagon_nn_->hexagon_nn_append_node(graph_id_,     
                op_id,   OP_QuantizedAdd_8p8to8,   NN_PAD_VALID,   addlayer_input.data(), addlayer_input.size(),   addlayer_output.data(), addlayer_output.size());

        accu_conv_opid = op_id;

        op_id++;

        addlayer_input.resize(0);
        addlayer_output.resize(0);
    ...
    for(int i=0;i<inputSplitTensor->elementSize();i++)
        inputPtr[i] = inputSplitTensor->host<int8_t>()[i] + 128;
    ...
    for(int n = 0;n<split_num;n++) {
        {
            ...
            input_tensor.data = inputPtr + n*inputSplitTensor->elementSize() / split_num;
            input_tensor.dataLen = inputSplitTensor->elementSize() / split_num;
            input_tensor.data_valid_len = inputSplitTensor->elementSize() / split_num;
            input_tensor.batches = inputs[0]->batch();
            input_tensor.height = inputs[0]->height();
            input_tensor.width = inputs[0]->width();
            input_tensor.depth = inputs[0]->channel() / split_num;
        }

        {
            input_tensors.emplace_back();
            auto& input_tensor = input_tensors.back();
            input_tensor.data = inputs[1]->host<uint8_t>() + n*inputs[1]->elementSize() / split_num;
            input_tensor.dataLen = inputs[1]->elementSize() / split_num;
            input_tensor.data_valid_len = inputs[1]->elementSize() / split_num;
            input_tensor.batches = inputs[1]->batch() / split_num;
    ...
    if(mOp->type() == OpType_NITI_DSP_GRADIENTCONV_Int8) {
        for(int i=0;i<outputs[0]->elementSize();i++) {
...
ErrorCode NITI_DSPGradientSplitBatchConv_Int8::GlobalExecute(const std::vector<Tensor*>& inputs, const std::vector<Tensor*>& outputs) {
    const auto input = inputs[0];
    const auto weight = inputs[1];
...

    {
        ...
        memcpy(inputs[0]->host<uint8_t>(), inputSplitTensor->host<uint8_t>(), inputs[0]->elementSize());
   ...
   hexagon_nn_ = static_cast<CPUBackend*>(backend())->global_hexagon_nn_;
    ...
    for (int n=0; n<split_num; n++) {
        ...
        outdiffTransposeShape.push_back(0);
        outdiffTransposeShape.push_back(3);
        addInputTransposeLayerWithInputOpIdWithIdxWithShape(hexagon_nn_,graph_id_, input_op_id, op_id,inputs[1], static_cast<CPUBackend*>(backend())->current_input_num, outdiff_transposelayer_input, outdiff_transposelayer_output, outdiffTransposeShape, &input_min, &input_max, actual_weight);
        static_cast<CPUBackend*>(backend())->current_input_num++;
...
int NITI_DSPGradientSplitBatchConv_Int8::conv_num = 0;

class NITI_DSPGradientSplitBatchConv_Int8_Creator : public CPUBackend::Creator {
public:
    virtual Execution* onCreate(const std::vector<Tensor*>& inputs, const std::vector<Tensor*>& outputs,
                                const MNN::Op* op, Backend* backend) const override {

        auto convOp = op->main_as_NITI_CONV_Int8();
        return new NITI_DSPGradientSplitBatchConv_Int8(backend, convOp, op);
    }
};

REGISTER_CPU_OP_CREATOR(NITI_DSPGradientSplitBatchConv_Int8_Creator, OpType_NITI_DSP_GRADIENT_SPLITBatchCONV_Int8);
} // namespace MNN
