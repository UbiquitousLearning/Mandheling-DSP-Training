//
//  NITI_DSPGradientSplitBatchConv_Int8.hpp
//  MNN
//
...
#ifndef NITI_DSPGradientSplitBatchConv_Int8_hpp
#define NITI_DSPGradientSplitBatchConv_Int8_hpp
...
class NITI_DSPGradientSplitBatchConv_Int8 : public CPUConvolution {
public:
    
    NITI_DSPGradientSplitBatchConv_Int8(Backend *backend, const NITI_CONV_Int8* common,  const MNN::Op* op);
    virtual ~NITI_DSPGradientSplitBatchConv_Int8();
...
private:
    ...
    std::shared_ptr<Tensor> inputSplitTensor;
    ...
    const int minibatch = 4;

    std::vector<int32_t> convOutputOpid;

    std::vector<hexagon_nn_input> addlayer_input;
    std::vector<hexagon_nn_output> addlayer_output;
