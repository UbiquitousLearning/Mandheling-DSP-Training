...
#include <math.h>
#include "backend/cpu/NITI_DSPReshapeGrad_Int8.hpp"
...
ErrorCode NITI_DSPReshapeGrad_Int8::onResize(const std::vector<Tensor *> &inputs, const std::vector<Tensor *> &outputs) {

    return NO_ERROR;
}

ErrorCode NITI_DSPReshapeGrad_Int8::onExecute(const std::vector<Tensor *> &inputs, const std::vector<Tensor *> &outputs) {
    ...
    desired_shape.push_back(obatch);
    desired_shape.push_back(oheight);
    desired_shape.push_back(owidth);
    desired_shape.push_back(ochannel);
...
ErrorCode NITI_DSPReshapeGrad_Int8::GlobalExecute(const std::vector<Tensor *> &inputs, const std::vector<Tensor *> &outputs) {
    ...
    int input_op_id = backend()->get_Op_id(inputs[0], OpType_NITI_DSP_RESHAPEGrad_Int8);

    reshapelayer_input.push_back(hexagon_nn_input());
    reshapelayer_input.back().src_id = input_op_id;
    reshapelayer_input.back().output_idx = 0;
...
NITI_DSPReshapeGrad_Int8::NITI_DSPReshapeGrad_Int8(Backend *b, const MNN::Op* op) : MNN::Execution(b) {
    // nothing to do
    if(!getDSPExecuteMode())
...
class NITI_DSPReshapeGrad_Int8_Creator : public CPUBackend::Creator {
public:
    virtual Execution *onCreate(const std::vector<Tensor *> &inputs, const std::vector<Tensor *> &outputs,
                                const MNN::Op *op, Backend *backend) const override {
        return new NITI_DSPReshapeGrad_Int8(backend, op);
    }
};

REGISTER_CPU_OP_CREATOR(NITI_DSPReshapeGrad_Int8_Creator, OpType_NITI_DSP_RESHAPEGrad_Int8);
