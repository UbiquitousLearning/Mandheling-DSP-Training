...
#include "backend/cpu/NITI_Conv_Int8.hpp"
...
bool NITI_Conv_Int8::reorderWeight(Backend* bn, const Convolution2DCommon* common,
                          Tensor* weightOrigin,
...
NITI_Conv_Int8::~NITI_Conv_Int8() {
    // Do nothing
}
NITI_Conv_Int8::NITI_Conv_Int8(Backend* backend, const NITI_CONV_Int8* convOp) : CPUConvolution(convOp->common(), backend) {
...
ErrorCode NITI_Conv_Int8::onResize(const std::vector<Tensor*>& inputs, const std::vector<Tensor*>& outputs) {
...
ErrorCode NITI_Conv_Int8::onExecute(const std::vector<Tensor*>& inputs, const std::vector<Tensor*>& outputs) {
    ...
    auto threadFunction = [&](int tId) {
        ...
        for (int bIndex = begin; bIndex < end; ++bIndex) {
            ...
            for (int tIndex = 0; tIndex < mTileCount; tIndex += 1) {
                ...
                // MNN_PRINT("NITI_Conv_Int8 execute mGemmKernel\n");
                // MNN_PRINT("NITI_Conv_Int8 execute im2col %p %p\n", colAddr, weightDataPtr);
    ...
    if(shift > 1) {
        ...
        NITI_MNNPstoShiftInt32(outputDataPtr, 2, 
            output_int32_ptr, obatch*ochannel*owidth*oheight);
        
        // cast成int8
        for(int i=0;i<osize;i++) {
            output_ptr[i] =  (int8_t)output_int32_ptr[i];
        }
        ...
        // cast成int8
        for(int i=0;i<osize;i++) {
            output_ptr[i] =  (int8_t)outputDataPtr[i];
        }
    }

    *(outputs[1]->host<int8_t>()) = (int8_t)exp_out;

    return NO_ERROR;
}

class NITI_Conv_Int8_Creator : public CPUBackend::Creator {
public:
    virtual Execution* onCreate(const std::vector<Tensor*>& inputs, const std::vector<Tensor*>& outputs,
                                const MNN::Op* op, Backend* backend) const override {

        auto convOp = op->main_as_NITI_CONV_Int8();
        return new NITI_Conv_Int8(backend, convOp);
    }
};

REGISTER_CPU_OP_CREATOR(NITI_Conv_Int8_Creator, OpType_NITI_CONV_Int8);
