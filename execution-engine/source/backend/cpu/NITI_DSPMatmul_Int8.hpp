...
#ifndef NITI_DSPMatmul_Int8_hpp
#define NITI_DSPMatmul_Int8_hpp
...
class NITI_DSPMatmul_Int8 : public CPUConvolution {
public:
    
    NITI_DSPMatmul_Int8(Backend *backend, const NITI_CONV_Int8* common,  const MNN::Op* op);
    virtual ~NITI_DSPMatmul_Int8();
...
private:
    ...
    std::vector<hexagon_nn_input> im2collayer_input;
    std::vector<hexagon_nn_output> im2collayer_output;
    ...
    std::vector<int32_t> weight_desired_shape;
    std::vector<hexagon_nn_input> weight_reshapelayer_input;
    std::vector<hexagon_nn_output> weight_reshapelayer_output;


    std::vector<int32_t> output_desired_shape;
    std::vector<hexagon_nn_input> output_reshapelayer_input;
    std::vector<hexagon_nn_output> output_reshapelayer_output;

    std::vector<hexagon_nn_input> requantizelayer_input;
    std::vector<hexagon_nn_output> requantizelayer_output;

    std::shared_ptr<Tensor> matmulA;
    std::shared_ptr<Tensor> matmulB;
    std::shared_ptr<Tensor> matmulC;

    std::vector<hexagon_nn_input> empty_input_list;

};

} // namespace MNN

#endif /* NITI_DSPMatmulGradientConv_Int8_hpp */
