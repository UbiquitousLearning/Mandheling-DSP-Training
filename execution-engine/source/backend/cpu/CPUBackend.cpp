...
CPUBackend::CPUBackend(const CPURuntime* runtime, BackendConfig::PrecisionMode precision, MNNForwardType type, size_t flags) : Backend(type) {
    ...
    if(getDSPExecuteMode()) {
        if(!build_graph) {
            global_hexagon_nn_ = generate_interface();
            initDspGraph(global_hexagon_nn_, global_graph_id_);
            initDspGraph(global_hexagon_nn_, split_gradient_graph_id_);
        }
...
int CPUBackend::get_Op_id(Tensor* t, MNN::OpType type) {
    int input_op_id = opadr_opid_map[t->host<int8_t>()];
    if(input_op_id == 0 || input_op_id == -1) {
        MNN_ERROR("error op type = %d\n", type);
        exit(-1);
    }
...
void CPUBackend::insert_Op_id(Tensor* t, int op_id) {
    if(opadr_opid_map.count(t->host<int8_t>())) {
        opadr_opid_map[t->host<int8_t>()] = op_id;
    } else {
        opadr_opid_map.insert(std::make_pair( t->host<int8_t>(), op_id ));
    }
}

void CPUBackend::CPU_P_DSPExecute() {

    if(build_graph) {

        for(int i=0;i<1;i++) {
            int error = global_hexagon_nn_->hexagon_nn_execute_new( global_gradient_graph_id_, gradient_input_tensors.data(), gradient_input_tensors.size(),
            gradient_output_tensors.data(), gradient_output_tensors.size());
        ...
        for(auto& iter : gradient_map) {
            for(int i=0;i<iter.second;i++) {
                // iter.first[i] = *((uint8_t*)iter.first + i) - 128;

                iter.first[i] = iter.first[i]/16;
            }
        }

    } else {
        global_hexagon_nn_->hexagon_nn_append_node(global_gradient_graph_id_,     
                global_op_id,   OP_OUTPUT,   NN_PAD_NA,   gradient_outputlayer_input.data(), gradient_outputlayer_input.size(),   NULL, 0);
        
        int error = global_hexagon_nn_->hexagon_nn_prepare(global_gradient_graph_id_);
        if(error!=0){
        ...
        for(int i=0;i<1;i++) {
            error = global_hexagon_nn_->hexagon_nn_execute_new( global_gradient_graph_id_, gradient_input_tensors.data(), gradient_input_tensors.size(),
            gradient_output_tensors.data(), gradient_output_tensors.size());
...
void CPUBackend::CPU_Split_DSPExecute() {

    if(build_graph) {

        long begin = getCurrentTime();
        for(int i=0;i<1;i++) {
            int error = global_hexagon_nn_->hexagon_nn_execute_new( split_gradient_graph_id_, gradient_input_tensors.data(), gradient_input_tensors.size(),
            gradient_output_tensors.data(), gradient_output_tensors.size());
        ...
        global_hexagon_nn_->hexagon_nn_append_node(split_gradient_graph_id_,     
                global_op_id,   OP_OUTPUT,   NN_PAD_NA,   gradient_outputlayer_input.data(), gradient_outputlayer_input.size(),   NULL, 0);
        
        int error = global_hexagon_nn_->hexagon_nn_prepare(split_gradient_graph_id_);
        if(error!=0){
            MNN_ERROR("Whoops... Cannot prepare: %d\n", error);
            exit(-1);
        }
        ...
        long begin = getCurrentTime();
        for(int i=0;i<1;i++) {
            error = global_hexagon_nn_->hexagon_nn_execute_new( split_gradient_graph_id_, gradient_input_tensors.data(), gradient_input_tensors.size(),
            gradient_output_tensors.data(), gradient_output_tensors.size());
...
void CPU_P_DSPExecute_func(CPUBackend *b) {
    b->CPU_P_DSPExecute();
}

void CPUBackend::DSPParallelWait() {
    if(!parallel_mode)
        return;
    if(!threads[0].joinable()) {
        MNN_PRINT("thread no support joinable\n");
        return;
    }
    threads[0].join();
...
void CPUBackend::DSPExecute()  {

    if(getDSPExecuteMode() && parallel_mode) {
        threads[0] = std::thread(CPU_P_DSPExecute_func, this);

        return ;
    }

    if(getDSPExecuteMode() && split_mode) {
        
        CPU_Split_DSPExecute();
        FILE* f = fopen("DSP.txt","w");
        fprintf(f, "%d\n",0);
        fclose(f);
        return ;
    ...
    if(!execute_mode)
    {
        ...
        global_hexagon_nn_->hexagon_nn_append_node(global_graph_id_,     
                global_op_id,   OP_OUTPUT,   NN_PAD_NA,   outputlayer_input.data(), outputlayer_input.size(),   NULL, 0);
        
        int error = global_hexagon_nn_->hexagon_nn_prepare(global_graph_id_);
        if(error!=0){
            MNN_ERROR("Whoops... Cannot prepare: %d\n", error);
            exit(-1);
        }
        ...
        for(int i=0;i<1;i++) {
            error = global_hexagon_nn_->hexagon_nn_execute_new( global_graph_id_, input_tensors.data(), input_tensors.size(),
        ...
        global_hexagon_nn_->hexagon_nn_teardown(global_graph_id_);
        ...
        if(!build_graph) {
            ...
            for(int i=0;i<1;i++) {
                int error = global_hexagon_nn_->hexagon_nn_execute_new( global_graph_id_, input_tensors.data(), input_tensors.size(),
                output_tensors.data(), output_tensors.size());
...
bool CPUBackend::isDSPParallel() {
    return parallel_mode;
}

hexagon_nn_nn_id CPUBackend::global_graph_id_ = 0;
hexagon_nn_nn_id CPUBackend::global_gradient_graph_id_ = 0;
hexagon_nn_nn_id CPUBackend::split_gradient_graph_id_ = 0;
const HexagonNN*  CPUBackend::global_hexagon_nn_ = NULL;
bool CPUBackend::build_graph = false;
bool CPUBackend::execute_mode = false;
bool CPUBackend::parallel_mode = false;
bool CPUBackend::split_mode = false;
