//
//  NITI_ConvMinMax_Int8.cpp
//  MNN
//
//  Created by xudaliang on 2021/11/04.
//  
//

#include "backend/cpu/NITI_ConvMinMax_Int8.hpp"
...
bool NITI_ConvMinMax_Int8::reorderWeight(Backend* bn, const Convolution2DCommon* common,
                          Tensor* weightOrigin,
...
NITI_ConvMinMax_Int8::~NITI_ConvMinMax_Int8() {
    // Do nothing
}
NITI_ConvMinMax_Int8::NITI_ConvMinMax_Int8(Backend* backend, const NITI_CONV_Int8* convOp) : CPUConvolution(convOp->common(), backend) {
...
ErrorCode NITI_ConvMinMax_Int8::onResize(const std::vector<Tensor*>& inputs, const std::vector<Tensor*>& outputs) {
...
ErrorCode NITI_ConvMinMax_Int8::onExecute(const std::vector<Tensor*>& inputs, const std::vector<Tensor*>& outputs) {
    ...
    NITI_RangeMinMax(outputDataPtr, osize, range_min, range_max);
    

    *(outputs[1]->host<int32_t>()) = range_min;
    *(outputs[2]->host<int32_t>()) = range_max;
    ...
    NITI_MNNShiftInt32(outputDataPtr, shift, output_int32_ptr,
                osize);

    return NO_ERROR;
}

class NITI_ConvMinMax_Int8_Creator : public CPUBackend::Creator {
public:
    virtual Execution* onCreate(const std::vector<Tensor*>& inputs, const std::vector<Tensor*>& outputs,
                                const MNN::Op* op, Backend* backend) const override {

        auto convOp = op->main_as_NITI_CONV_Int8();
        return new NITI_ConvMinMax_Int8(backend, convOp);
    }
};

REGISTER_CPU_OP_CREATOR(NITI_ConvMinMax_Int8_Creator, OpType_NITI_CONVMaxMin_Int8);
