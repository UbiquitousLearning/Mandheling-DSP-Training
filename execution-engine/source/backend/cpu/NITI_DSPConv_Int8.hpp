...
#ifndef NITI_DSPConv_Int8_hpp
#define NITI_DSPConv_Int8_hpp
...
class NITI_DSPConv_Int8 : public CPUConvolution {
public:
    
    NITI_DSPConv_Int8(Backend *backend, const NITI_CONV_Int8* common,  const MNN::Op* op);
    virtual ~NITI_DSPConv_Int8();
...
private:
    ...
    std::vector<hexagon_nn_input> input_castlayer_input;
    std::vector<hexagon_nn_output> input_castlayer_output;

    std::vector<hexagon_nn_input> weight_castlayer_input;
    std::vector<hexagon_nn_output> weight_castlayer_output;
