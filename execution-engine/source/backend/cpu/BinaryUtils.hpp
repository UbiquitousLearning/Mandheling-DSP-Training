...
template<typename Tin0, typename Tin1, typename Func>
void niti_execute(void* outputRaw, const void* inputRaw0, const void* inputRaw1, int elementSize, int broadcastIndex) {
    Func f;
    const int input0DataCount = elementSize;
    const int input1DataCount = elementSize;
    const Tin0* input0Data = (const Tin0*)inputRaw0;
    const Tin1* input1Data = (const Tin1*)inputRaw1;
    int8_t* outputData = (int8_t*)outputRaw;

    if (broadcastIndex == 0) { // data count == 1, not only mean scalar input, maybe of shape (1, 1, 1, ...,1)
        for (int i = 0; i < input1DataCount; i++) {
            outputData[i] = (int8_t)NITI_int8_clip(f((int32_t)input0Data[0], (int32_t)input1Data[i]));
        }
    } else if (broadcastIndex == 1) {
        for (int i = 0; i < input0DataCount; i++) {
            outputData[i] = (int8_t)NITI_int8_clip(f((int32_t)input0Data[i], (int32_t)input1Data[0]));
        }
    } else { // both input contains more than one element，which means no scalar input
        for (int i = 0; i < input0DataCount; i++) {
            outputData[i] = (int8_t)NITI_int8_clip(f((int32_t)input0Data[i], (int32_t)input1Data[i]));
        }
    }
}
