...
void NITI_MNNPackForMatMul_B_Int8(int8_t* dest, const int8_t* source, size_t h, size_t l, bool transpose, bool packC4) {
    ...
    auto hR = hP * 4;
    if (hR != h) {
        ::memset(dest, 0, UP_DIV(h, 4)*4*l*sizeof(int8_t));
    }
    if (transpose) {
        for (int y=0; y<hP; ++y) {
            auto destY = dest + y * 4 * l;
            auto sourceY = source + y * 4;
            for (int x=0; x<l; ++x) {
                ::memcpy(destY + 4 * x, sourceY + x * h, 4 * sizeof(int8_t));
            }
        }
        auto hRemain = h - hR;
        if (hRemain > 0) {
            ...
            auto sourceY = source + hP * 4;
            for (int x=0; x<l; ++x) {
                ::memcpy(destY + 4 * x, sourceY + x * h, hRemain * sizeof(int8_t));
            }
        }
    }
    int offset[] = {
    ...
    if(packC4)
        NITI_MNNPackC4_Int8(dest, source, l, h, offset);
}
...
int32_t NITI_RangeEstimate(int32_t* input, int size) {
    int32_t int32_bitwidth = 0;
    for(int i=0;i<size; i++) {
        // MNN_PRINT("output32 = %d\n", input[i]);
        if(int32_bitwidth < abs(input[i]))
            int32_bitwidth = abs(input[i]);
    }
    if(int32_bitwidth == 0)
        return 0;
    else
        return ceil(log2(int32_bitwidth));
}
...
void NITI_MNNPstoShiftInt32(int32_t* input, int shift, int32_t* dest, int size) {
    ...
    if(shift % 2 == 1) {
        for(int i=0;i<size;i++) {
            ...
            dest[i] = NITI_int8_clip(round_temp + round_1*sign_1);
        }
    } else {
        for(int i=0;i<size;i++) {
            int32_t round_temp = input[i] / (1 << shift);
    ...
    for(int i=0;i<size;i++) {
        int32_t round_temp = input[i] / (1 << shift);
        dest[i] = NITI_int8_clip(round_temp);
    }
#endif
}
...
void NITI_MNNStoShiftInt32(int32_t* input, int8_t* dest, int shift, int size) {
    for(int i=0;i<size;i++) {
        int input_data = input[i];
        int round_temp = input_data / (1 << shift);
        int prob = abs(input_data - (round_temp *  (1 << shift) ));
        int rand_num = rand() % (1<<shift);

        int round_decision = (prob > rand_num);
        round_decision = round_decision * NITI_sign(input_data);
        dest[i] = (int8_t)NITI_int8_clip(round_temp + round_decision);
    }
}
...
void NITI_MNNPackTranspose_Int8(int32_t* dst, const int32_t* src, size_t area, size_t depth, int* areaOffset) {
    ...
    for (int hi = 0; hi < area; ++hi) {
        const int32_t* srcHeight = src + hi * 4;
        int32_t* dstHeight       = dst + hi * c;
        for (int ci = 0; ci < cDiv4; ++ci) {
            Vec4::save((float*)(dstHeight + 4 * ci), Vec4::load(srcHeight + 4 * ci * srcArea));
        }
    }
    ...
    for (int hi = 0; hi < area; ++hi) {
        const int32_t* srcHeight = srcAlign + hi * 4;
        int32_t* dstHeight       = dstAlign + hi * c;
