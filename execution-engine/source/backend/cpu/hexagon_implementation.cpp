...
#include "MNN/hexagon/hexagon_nn/hexagon_implementation.h"
...
#include "MNN/hexagon/hexagon_nn/hexagon_nn_interface.h"

#include "core/Macro.h"
namespace MNN {

int DSPExecuteMode = false;

// 用于表示当前是否使用贯穿算子计算
    // true 贯穿, false 独立
bool getDSPExecuteMode() {
    FILE *f = fopen("DSP.txt","r");
    fscanf(f, "%d\n",&DSPExecuteMode);
    fclose(f);

    return DSPExecuteMode==1;
}

void setDSPExecuteMode(bool mode) {
    DSPExecuteMode = mode;
}
...
HexagonNN CreateNewHexagonInterface() {
  ...
  void* libhexagon_interface =
      dlopen("libhexagon_interface_arm64.so", RTLD_LAZY | RTLD_LOCAL);
  if (libhexagon_interface == nullptr) {
    MNN_ERROR("Failed to load libhexagon_interface.so, Error: %s",
                    dlerror());
    return hexagon_nn;
  }
