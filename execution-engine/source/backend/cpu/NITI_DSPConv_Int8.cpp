...
#include "backend/cpu/NITI_DSPConv_Int8.hpp"
...
NITI_DSPConv_Int8::~NITI_DSPConv_Int8() {
    // Do nothing
    // hexagon_nn_->hexagon_nn_global_teardown();
}
NITI_DSPConv_Int8::NITI_DSPConv_Int8(Backend* backend, const NITI_CONV_Int8* convOp, const MNN::Op* op) : CPUConvolution(convOp->common(), backend) {
...
ErrorCode NITI_DSPConv_Int8::onResize(const std::vector<Tensor*>& inputs, const std::vector<Tensor*>& outputs) {
    ...
    backend()->onReleaseBuffer(inputUINT8Tensor.get(), Backend::DYNAMIC);
     ...
     if(getDSPExecuteMode() && !static_cast<CPUBackend*>(backend())->build_graph) {
        ...
        if(ichannel <= 4) {
            ...
            auto& max_sizes0 = static_cast<CPUBackend*>(backend())->_outputs0.back().max_sizes;
            for (int i = 0; i < 4; ++i) {
            ...
            static_cast<CPUBackend*>(backend())->total_input_num+=2;
...
ErrorCode NITI_DSPConv_Int8::onExecute(const std::vector<Tensor*>& inputs, const std::vector<Tensor*>& outputs) {
    ...
    hexagon_nn_->hexagon_nn_append_const_node(
        ...
        op_id++,                    // Node identifier (a unique uint32)
        1,                          // size: batches
        mCommon->strideX(),                          // size: height
        mCommon->strideY(),                          // size: width
        1,                          // size: depth
...
ErrorCode NITI_DSPConv_Int8::GlobalExecute(const std::vector<Tensor*>& inputs, const std::vector<Tensor*>& outputs) {
    ...
    if(inputs[0]->channel() <= 4 ) {
        ...
        hexagon_nn_->hexagon_nn_append_node(graph_id_,     
            op_id,   OP_INPUT,   NN_PAD_NA,   empty_input.data(), 0,   static_cast<CPUBackend*>(backend())->_outputs0.data(), static_cast<CPUBackend*>(backend())->_outputs0.size());


        addInputCastLayerWithIdx(hexagon_nn_, graph_id_, 0x1000, op_id, inputs[0], static_cast<CPUBackend*>(backend())->current_input_num, input_castlayer_input, input_castlayer_output, OP_Quantized_CastInt8ToUInt8);
        static_cast<CPUBackend*>(backend())->current_input_num++;

        static_cast<CPUBackend*>(backend())->opadr_opid_map.insert(std::make_pair(inputs[0]->host<int8_t>(), op_id ));
        ...
        static_cast<CPUBackend*>(backend())->opadr_opid_map.insert(std::make_pair(inputs[1]->host<int8_t>(), op_id ));

        static_cast<CPUBackend*>(backend())->weight_map.insert(std::make_pair( (int8_t*)weightTensorPtr, inputs[1]->elementSize() ));
        ...
        int input_op_id = backend()->get_Op_id(inputs[0], OpType_NITI_DSP_CONV_Int8);
...
class NITI_DSPConv_Int8_Creator : public CPUBackend::Creator {
public:
    virtual Execution* onCreate(const std::vector<Tensor*>& inputs, const std::vector<Tensor*>& outputs,
                                const MNN::Op* op, Backend* backend) const override {

        auto convOp = op->main_as_NITI_CONV_Int8();
        return new NITI_DSPConv_Int8(backend, convOp, op);
    }
};

REGISTER_CPU_OP_CREATOR(NITI_DSPConv_Int8_Creator, OpType_NITI_DSP_CONV_Int8);
} // namespace MNN
