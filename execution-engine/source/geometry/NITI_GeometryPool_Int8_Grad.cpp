...
namespace MNN {
class NITI_GeometryPool_Int8_Grad : public GeometryComputer {
public:
    ...
    bool onComputeMaxPool(const Op* op, const std::vector<Tensor*>& inputs, const std::vector<Tensor*>& outputs,
        ...
        auto parameter = op->main_as_NITI_Pool_Int8();
        ...
        for (int ky = 0; ky < kernel_h; ky++) {
            ...
            for (int kx = 0; kx < kernel_w; kx++) {
                ...
                // BinaryOperation是支持int32 和 float 两种类型的，无需要重定义算子
        ...
        int oob = outputs[0]->batch();
        int ooc = outputs[0]->channel();
        int ooh = outputs[0]->height();
        int oow = outputs[0]->width();
        // eltwise
        std::shared_ptr<Tensor> tmpOutput(new Tensor);
        {
            tmpOutput->buffer().type       = halide_type_of<float>();
            tmpOutput->buffer().dimensions = 4;
            tmpOutput->setLength(0, oob);
            tmpOutput->setLength(1, ooc);
            tmpOutput->setLength(2, ooh);
            tmpOutput->setLength(3, oow);
        ...
        // POOL不需要做round shift!!!!
        // 中间结果是32位的算子，最后结果需要cast
        {
            std::unique_ptr<OpT> cast2int8(new OpT);
            cast2int8->type                   = OpType_Cast;
            cast2int8->main.type                = OpParameter_CastParam;
            cast2int8->main.value               = new CastParamT;
            cast2int8->main.AsCastParam()->srcT = DataType_DT_INT32;
            cast2int8->main.AsCastParam()->dstT = DataType_DT_INT8;
        
            flatbuffers::FlatBufferBuilder builder;
            auto lastOffset = Op::Pack(builder, cast2int8.get());
            builder.Finish(lastOffset);
            Command cmd;
            cmd.buffer.resize(builder.GetSize());
            ::memcpy(cmd.buffer.data(), builder.GetBufferPointer(), cmd.buffer.size());
            cmd.inputs.resize(1);
            cmd.inputs[0] = tmpOutput.get();
            cmd.outputs = outputs;
            cmd.op      = flatbuffers::GetMutableRoot<Op>(cmd.buffer.data());
...
REGISTER_GEOMETRY(NITI_GeometryPool_Int8_Grad, _create);
