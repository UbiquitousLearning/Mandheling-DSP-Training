...
MNN_PUBLIC std::vector<VARP>  _NITI_Conv_Int8(std::vector<int8_t>&& weight, VARP ascale, int8_t wscale, VARP x, INTS channel, INTS kernelSize,
                              PaddingMode pad, INTS stride, INTS dilate, int group, INTS pads, int nbits);
MNN_PUBLIC std::vector<VARP>  _NITI_Conv_Int8(VARP weight, VARP bias, VARP scale, VARP x,
                              PaddingMode pad, INTS stride, INTS dilate, int group, INTS pads, int nbits);
MNN_PUBLIC std::vector<VARP>  _NITI_Maxpool_Int8(VARP x, VARP ascale, INTS kernel, INTS stride = {1, 1}, PaddingMode pad = VALID, INTS pads= {0, 0});
MNN_PUBLIC VARP _NITI_Softmax(VARP logits, VARP ascale, int axis);
MNN_PUBLIC VARP  _NITI_LOSS_SUM(VARP logits, VARP ascale, VARP oneHotTargets, int batch, int channel, int height, int width);
MNN_PUBLIC VARP _NITI_Relu_Int8(VARP x);
MNN_PUBLIC VARP _NITI_ReluGrad_Int8(VARP input, VARP x);
MNN_PUBLIC VARP _NITI_Pad_Int8(VARP input, int padpos);
MNN_PUBLIC VARP _NITI_ConvertChannel1NC4HW4ToNCHW_Int8(VARP x);
MNN_PUBLIC std::vector<VARP> _NITI_ConvMinMax_Int8(VARP weight, VARP ascale, VARP wscale, VARP x,
                              PaddingMode pad, INTS stride, INTS dilate, int group, INTS pads, int nbits);
MNN_PUBLIC VARP _NITI_LeftPoolGrad(VARP outputdiff, INTS shape, INTS stride );
MNN_PUBLIC std::vector<VARP>  _NITI_DSP_Conv_Int8(VARP weight, VARP bias, VARP scale, VARP x,
                              PaddingMode pad, INTS stride, INTS dilate, int group, INTS pads, int nbits);
MNN_PUBLIC std::vector<VARP>  _NITI_DSP_Conv_Int8_SIMU(VARP weight, VARP bias, VARP scale, VARP x, VARP min, VARP max,
                              PaddingMode pad, INTS stride, INTS dilate, int group, INTS pads, int nbits);
MNN_PUBLIC VARP _NITI_DSP_Relu_Int8(VARP x);
MNN_PUBLIC std::vector<VARP> _NITI_DSP_Maxpool_Int8(VARP x, VARP ascale, INTS kernel, INTS stride = {1, 1}, PaddingMode pad = VALID, INTS pads= {0, 0});
MNN_PUBLIC VARP _NITI_DSP_ReshapeGrad(VARP x, INTS shape, Dimensionformat original_format = NHWC);
MNN_PUBLIC VARP _NITI_DSP_Reshape(VARP x, INTS shape, Dimensionformat original_format = NHWC);
MNN_PUBLIC VARP _NITI_DSP_ReluGrad_Int8(VARP input, VARP x);
MNN_PUBLIC VARP _NITI_DSP_Transpose(VARP x, INTS perm);
MNN_PUBLIC VARP _NITI_DSP_Transpose(VARP x, VARP perm);
MNN_PUBLIC VARP _NITI_DSP_WeightRotate180_Int8(VARP input);
MNN_PUBLIC VARP _NITI_DSP_Pad_Int8(VARP input, int padpos);
MNN_PUBLIC VARP _NITI_DSP_LeftPoolGrad_Deconv(VARP x, INTS shape, INTS stride );
MNN_PUBLIC VARP _NITI_DSP_LeftPoolGrad_Gradient(VARP x, INTS shape, INTS stride );
MNN_PUBLIC VARP _NITI_DSP_Binary_Add(VARP x, VARP y);
MNN_PUBLIC VARP _NITI_DSP_NOP(VARP x);
MNN_PUBLIC VARP _NITI_Transpose_INT8(VARP x, INTS perm);
} // namespace Express
} // namespace MNN
